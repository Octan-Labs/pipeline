...
dag = DAG('Pre_Tx_And_Volume_Calculation_Spark_Job',
...
pre_tx_calculation = KubernetesPodOperator(
            ...
            cmds=[
              ...
              "s3a://datateam-spark/jobs/pre-tx-calculation.py",
              "-b={{ dag_run.conf['base_path'] }}",
              "-d={{ dag_run.conf['date'] }}",
              "-n={{ dag_run.conf['name'] }}",
              "-s={{ dag_run.conf['symbol'] }}"
            ],
            name=f"pre-tx-calculation",
            task_id=f"pre-tx-calculation",
            retries=5,
            retry_delay=timedelta(minutes=5),
            dag=dag,
        )

volume_calculation = KubernetesPodOperator(
            image="171092530978.dkr.ecr.ap-southeast-1.amazonaws.com/octan/sparkonk8s:0.0.19",
            cmds=[
              "/usr/bin/tini",
              "-s",
              ...
              "--conf",
              "spark.hadoop.mapreduce.fileoutputcommitter.algorithm.version=2",
              "s3a://datateam-spark/jobs/volume-calculation.py",
              "-b={{ dag_run.conf['base_path'] }}",
              "-d={{ dag_run.conf['date'] }}"
            ],
            name=f"volume-calculation",
            task_id=f"volume-calculation",
            retries=5,
            retry_delay=timedelta(minutes=5),
            dag=dag,
        )

pre_tx_calculation >> volume_calculation