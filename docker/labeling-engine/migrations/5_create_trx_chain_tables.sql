CREATE TABLE IF NOT EXISTS trx_reputations (
    block UInt32,
...
CREATE TABLE IF NOT EXISTS trx_top_reputations (
    rank UInt32,
    date DateTime,
    address String,
    reputation_score Float64,
...
ORDER BY address;
CREATE MATERIALIZED VIEW trx_top_reputations_mv TO trx_top_reputations AS WITH latest_reputations AS (
    SELECT address,
        max(date) as latest_date
    FROM trx_reputations
    GROUP BY address
)
...
FROM latest_reputations AS lr
    JOIN trx_reputations AS er ON lr.address = er.address
    AND lr.latest_date = er.date;
CREATE TABLE IF NOT EXISTS sorting_trx_top_reputations (
    rank UInt32,
    date DateTime,
    address String,
    reputation_score Float64,
