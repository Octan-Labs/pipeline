CREATE TABLE IF NOT EXISTS base_reputations (
    block UInt32,
...
CREATE TABLE IF NOT EXISTS base_top_reputations (
    rank UInt32,
    date DateTime,
    address FixedString(42),
    reputation_score Float64,
...
ORDER BY address;
CREATE MATERIALIZED VIEW base_top_reputations_mv TO base_top_reputations AS WITH latest_reputations AS (
    SELECT address,
        max(date) as latest_date
    FROM base_reputations
    GROUP BY address
)
...
FROM latest_reputations AS lr
    JOIN base_reputations AS er ON lr.address = er.address
    AND lr.latest_date = er.date;
CREATE TABLE IF NOT EXISTS sorting_base_top_reputations (
    rank UInt32,
    date DateTime,
    address FixedString(42),
    reputation_score Float64,
