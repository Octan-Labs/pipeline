CREATE TABLE IF NOT EXISTS arb_reputations (
    block UInt32,
...
CREATE TABLE IF NOT EXISTS arb_top_reputations (
    rank UInt32,
    date DateTime,
    address FixedString(42),
    reputation_score Float64,
...
ORDER BY address;
CREATE MATERIALIZED VIEW arb_top_reputations_mv TO arb_top_reputations AS WITH latest_reputations AS (
    SELECT address,
        max(date) as latest_date
    FROM arb_reputations
    GROUP BY address
)
...
FROM latest_reputations AS lr
    JOIN arb_reputations AS er ON lr.address = er.address
    AND lr.latest_date = er.date;
CREATE TABLE IF NOT EXISTS sorting_arb_top_reputations (
    rank UInt32,
    date DateTime,
    address FixedString(42),
    reputation_score Float64,
