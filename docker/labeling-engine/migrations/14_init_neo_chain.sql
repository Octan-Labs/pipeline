CREATE TABLE IF NOT EXISTS labeling_neo_addresses (
  id String,
  address String,
  identity String,
  category String,
...
ORDER BY address;
CREATE MATERIALIZED VIEW neo_top_reputations_mv TO neo_top_reputations AS WITH latest_reputations AS (
  SELECT address,
    max(date) as latest_date
  FROM neo_reputations
  GROUP BY address
)
...
FROM latest_reputations AS lr
  JOIN neo_reputations AS er ON lr.address = er.address
